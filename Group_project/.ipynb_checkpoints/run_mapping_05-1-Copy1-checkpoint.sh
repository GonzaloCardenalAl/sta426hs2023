...
# create a pipe to process all sequences in the experiment at once
rm fastq_data
mkfifo fastq_data

#Join all the fastq files in one pipe
cat $experimentFolder/fastq_pass/*.fastq > fastq_data &

mkdir "$experimentFolder/alignments"

echo "Performing genome alignment on NPO5-Scr1"
#Mapping with minimap2
minimap2 -ax splice -uf -k14 $referenceFile fastq_data > $experimentFolder/alignments/genomic-aln.sam
# Convert sam to bam
...
minimap2 -ax map-ont -N 15 $referenceTranscriptome fastq_data > $experimentFolder/alignments/transcriptomic-aln.sam
# Convert sam to bam
